...
const NavItem = ({value,isActive,showLabel}:NavItemProps) => {
    ...
    return (
        ...
        md:h-[50px] hover:bg-green-700 md:text-lg ${showLabel ? 'w-[200px]':'w-[60px]'}
        transition-all duration-200 ease-in-out text-white hover:text-white ${isActive ? 'text-white bg-green-700 text-xl' : ''}
        rounded-md
        `}>
...
const NavBar = () => {
   ...
   return (
    ...
    md:justify-between md:py-8 px-1 bg-green-500 relative h-[80px]
    '>
      ...
      <Button className='bg-green-700 hidden md:block p-2 rounded-md text-white' clickAction={()=>setOpenNavLabel((prev:boolean) => !prev)}>
